...
class CO2DistributedDataParallel(Module):
    ...
    Parameters:
        ...
        co2_base_algorithm (CO2BaseAlgorithm):
            ...
            CO2, results in significant speedups without accuracy loss. Either Stochastic Gradient Push
            (CO2BaseAlgorithm.SGP) (https://arxiv.org/abs/1811.10792) or LocalSGD (CO2BaseAlgorithm.LOCALSGD)
            (https://arxiv.org/abs/1808.07217) can be used here (default: CO2BaseAlgorithm.LOCALSGD)
    CO2 Parameters:
        ...
        outer_momentum_memory_efficient (bool):
            ...
            on the same iteration. Also, we skip the gossip step when co2_base_algorithm is sgp. We can skip these
            ...
            localsgd_frequency.
            We recommend setting this parameter to True when co2_base_algorithm is localsgd. In case of sgp, there
            is a tradeoff between extra memory usage which is double the memory occupied by the parameters, and extra
            time spent which is half the time taken up by an allreduce every outer_frequency iterations and we
    ...
    SGP Parameters:
        ...
        co2_sgp_average_params (bool):
            Whether to completely average the parameters when co2 is done instead of a partial averaging that
    ...
    def __init__(
        ...
        co2_sgp_average_params: bool = False,
        # Debugging Args
        verbose: bool = False,
        ...
        self.co2_sgp_average_params = co2_sgp_average_params

        self.localsgd = co2_base_algorithm == CO2BaseAlgorithm.LOCALSGD
        self.sgp = co2_base_algorithm == CO2BaseAlgorithm.SGP
        ...
        if self.co2 and not self.localsgd and not self.sgp:
            self.logger.warning("CO2 is being used without LocalSGD and SGP")
        ...
        if self.sgp:
            self._sgp_init(
                ...
                co2_sgp_average_params=co2_sgp_average_params,
            )
        ...
        self.co2_thread = threading.Thread(
                ...
                args=(self.parameter_ready, self.co2_end))
    ...
    def _should_perform_sgp_common(self) -> bool:
        return self.sgp and not self.overlap and not self._skip_averaging_memory_efficient_co2()
    ...
    def _should_allreduce_params(self) -> bool:
        ...
        return (self.sgp and self._should_perform_co2() and self.co2_sgp_average_params) or (
            self._should_perform_localsgd() and not self._skip_averaging_memory_efficient_co2()
        )
    ...
    def _maybe_copy_back_fp32_parameters(self, fp16_fp32_list: List[Tuple[torch.Tensor, torch.Tensor]]) -> None:
        ...
        if (
            self._should_perform_sgp() or self._should_allreduce_params() or self._should_perform_co2()
        ) and fp16_fp32_list:
            with torch.no_grad():
    ...
    def perform_co2(self, optimizer: torch.optim.Optimizer, fp32_params: Optional[torch.Tensor] = None) -> None:
        ...
        the base algorithm (SGP/ LocalSGD) and the outer momentum step. Since LocalSGD and the outer
            ...
            for group in optimizer.param_groups:
                for p in group["params"]:
                    ...
                    if offset + numel > self.portion_start and offset < self.portion_end:
                        ...
                        if self.co2_clip:
                            # print("Use outer momentum clipping")
    ...
    def __make_backward_hook(self) -> Callable[..., None]:
        ...
        def hook(*unused: Any) -> None:
            ...
            if self.sgp:
                ...
                if self.gossip_enable and self.overlap and not self._skip_averaging_memory_efficient_co2():
                    self._sgp_query_gossip_queue()
    ...
    def _CO2_AAR(self, parameter_ready, co2_end):
        while True:
    ...
    def _sgp_init(
        ...
        if self.process_rank % self.nprocs_per_node == 0:
            self.gossip_thread = threading.Thread(
                target=CO2DistributedDataParallel._sgp_gossip_target,
                args=(
                    self.dist_config,
                    self.gossip_flag,
                    self.train_flag,
    ...
    def state_dict(self) -> Dict[str, Union[torch.Tensor, bool]]:  # type: ignore
        state_dict = super(CO2DistributedDataParallel, self).state_dict()
        if self.sgp:
    ...
    def load_state_dict(self, state_dict: Dict[str, Union[torch.Tensor, bool]]) -> None:  # type: ignore
        ...
        super(CO2DistributedDataParallel, self).load_state_dict(cast(Dict[str, torch.Tensor], state_dict))
    ...
    def train(self, mode: bool = True) -> "CO2DistributedDataParallel":
        super(CO2DistributedDataParallel, self).train(mode)
        if self.sgp:
            self.gossip_enable = True
        return self

    def eval(self) -> "CO2DistributedDataParallel":
        super(CO2DistributedDataParallel, self).eval()
        if self.sgp:
        ...
        while True:
            ...
            try:
                with torch.cuda.stream(gossip_stream):
                    for dtype in gossip_params_by_dtype:
                        (ps_weight, ps_factor,) = CO2DistributedDataParallel._sgp_gossip_into_receive_buffer(
